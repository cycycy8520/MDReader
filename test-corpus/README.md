...
语料中引用的图片一律**不提交到仓库**（避免二进制膨胀）。一条命令生成全部五张：

```bash
pnpm gen:corpus-assets
```

> **为什么改成脚本生成（2026-08-18）**：这里原本只写「自行放置任意 PNG」，
> 结果实际情况是**一张都没放过** —— `full-gfm.md` 第 10 节四个用例长期显示破图，
> 其中 10.2「中文 + 空格路径」还是 DG 点名的头号坑。也就是说那条路径从来没被真实验证过，
> 只是看起来测过了。一句「自行放置」换不来验证，一个命令才能。
> 生成的图带边框与对角线：导出 PDF / 长图后能一眼看出有没有被裁掉或拉伸，纯色块做不到。

下表是脚本生成的清单（改脚本要同步改这里）：
