...
- **形态**：单实例（官方插件路由）、左栏为"最近列表"而非标签页（2026-08-19 起增设可切换的文件夹树视图，F20，最近列表仍是默认态）、安装体积 ≤ 25MB、热启动 ≤ 1s、内存分级预算（见 3.2）。
...
| F19 | 便携版 | 与安装版同一 exe；同级存在 `portable.marker` 时数据根目录改为 `<exe目录>\data\`，不碰 %APPDATA%、不写注册表、不注册关联；产物为 zip | P1 | M1 批次 2 |
| F20 | 文件夹模式 | 打开文件夹为项目：左栏在「最近 ⇄ 文件夹树」间互斥切换（栏头小图标）；树只显 Markdown 五后缀、按层懒加载、单击打开、当前文件自动 reveal；**严格只读**（无新建/重命名/删除）；记住上次文件夹与展开态 + 新增「最近文件夹」（皆存 AppData，**绝不向用户目录写状态**）；目录变更 notify 递归监听自动刷新；文件关联启动**不**自动挂载父目录（显式动作升级）。细节见 5.3.1 | P1 | 批次 5（v1.0 前评估随发） |
...
| ~~便携版（portable）~~ | **2026-08-18 用户决定纳入 V1**（原列为不做）。形态：zip 直跑，exe 同级放 `portable.marker` 即启用便携模式——数据写程序目录 `data\`（settings.json / recent.json / logs\），不碰 `%APPDATA%`、不写任何注册表、不注册文件关联。与安装版同一份 exe，仅靠标记文件切换数据根目录。实现见 F19 |
...
> 布局参考 DeepSeek 客户端的"左栏 + 主区"结构：左侧是**最近文件列表**（相当于会话列表；**不是标签页**，点击=打开/切换该文件，无"关闭"概念），主区是阅读区。（2026-08-19 修订：左栏增设第二视图「文件夹树」，与最近列表互斥切换，见 5.3.1/F20；未挂载文件夹时与旧行为完全一致。）视觉关键词：**克制、聚焦阅读、现代扁平、少量高级感细节**。
...
|---|---|---|---|
| 顶栏 | 高 **40px**，自绘 | `--md-topbar-h` / `h-topbar` | 左：菜单折叠钮 + 文件名（修改中文件带 ● 刷新指示 200ms 闪烁）；右：打开文件、打开文件夹（F20）、查找、大纲；整条空白区为窗口拖动区，双击最大化。（v0.6.1 修订：导出/分享钮按用户 2026-08-19 决定撤下——与右键菜单完全重复，导出/分享/打印一律走正文右键与 Ctrl+P；「更多」菜单同理不设） |
...
### 5.3.1 文件夹模式（F20，用户 2026-08-19 决定纳入）

> 定位：**查看器的项目视图，不是文件管理器**。对标取舍：互斥切换抄 Typora（窄栏最省空间）、懒加载与单击即开抄 VS Code、「记住上次 + 零痕迹」反着 Obsidian 抄（状态只进 AppData，绝不写 `.obsidian` 式目录）。

- **入口**（v0.6.1 扩充，用户反馈「只在正文右键里 = 反人类」）：**顶栏「打开文件夹」按钮**（最高频入口）；**左栏空白区右键**（附录 A.5：打开文件 / 打开文件夹 ▸）；正文右键菜单打开方式组「打开文件夹 ▸」（Ctrl+Shift+O，已登记 6.5）；把文件夹拖入窗口；空状态页文字按钮。文件关联/命令行打开单文件**绝不**自动挂载父目录（Typora 的教训——意外且费 IO），正文右键菜单（打开方式组）提供显式「把所在文件夹作为项目打开」。
- **视图互斥**：左栏同一时刻只显示一种视图。未挂载文件夹 = 现状最近列表；挂载后自动切树视图，栏头显示文件夹名 + 两枚 16px 图标（时钟=最近 / 树=文件夹）一键互切；来回切换不丢树的展开状态。过滤框（Ctrl+Shift+F）两个视图共用，树视图下过滤已加载节点名。
- **树行为**：只显示 SUPPORTED_EXTENSIONS 五后缀；文件夹在前，`Intl.Collator('zh', {numeric:true})` 排序；默认展开根的第一层；**单击即打开**（查看器无预览/固定之分）；当前文件高亮 = 最近列表同款选中态（5.3），并自动展开路径 reveal；空文件夹展开后一行淡字「无 Markdown 文件」（不做递归预扫描判空——与懒加载冲突）。行高 32px、缩进 16px/层，控件高度契约不变。
- **只读红线**：树的右键菜单仅 打开 / 在资源管理器中显示 / 复制路径。**没有**新建/重命名/删除/移动（红线 5）。
- **性能**：按层懒加载（展开才 readdir 该层）；内置忽略 `.git` / `.obsidian` / `node_modules` / 隐藏目录 / 符号链接（防循环）；单层 > 2000 项截断加一行提示；目录监听 notify `Recursive` + 防抖合并（复用 FR-06 的 300ms 防抖模型），事件到达只刷新受影响的已加载层。
- **持久化**（settings.json，三处契约同步）：`sidebarView`（recent/tree）、`folderRoot`、`folderExpanded`（展开路径集）、`recentFolders`（上限 12，与最近文件分开记；**唯一显示面 = 正文右键「打开文件夹 ▸」子菜单**，失效条目在点击挂载失败时自动剔除并出警示条）。全部 AppData，卸载随现有清理流程带走。
- **实现约束**：选文件夹 = 已有 dialog 插件 `open({directory:true})`（权限零新增）；列目录 = 新自定义 Rust 命令 std 递归（**不引入 tauri-plugin-fs**——不把通用 fs 命令面暴露给渲染层）；零新运行时依赖（红线 12 核验过：notify/clap/dialog 全是存量）。
...
|---|---|---|
| Ctrl+O | 打开文件对话框 | M1 |
| Ctrl+Shift+O | 打开文件夹（F20 文件夹模式） | 批次 5 |
| F11 | 专注阅读切换（隐左栏/大纲/顶栏工具区；原名禅模式，2026-08-18 用户点名更名——与功能编号 F11=导出 PDF 无关，此列是键名） | 批次 3 |
...
| F5 | 手动重新渲染 | M1 |
| Esc | 关闭最上层浮层（查找条/灯箱/大纲浮层/弹窗）；左栏过滤框聚焦时清空并失焦（FR-03，v1.0 起）；无浮层时退出专注阅读；专注阅读也未开启时无动作 | M1 |
...
| v0.6 | 2026-08-19 | **F20 文件夹模式纳入**（用户决定，流程照 F19 先例）：新增 2.1 F20 行与 5.3.1 规格节，0 节/5.1 概念模型注记修订（左栏 = 最近列表 + 可切换文件夹树，最近列表仍为默认态），6.5 登记 Ctrl+Shift+O；执行挂 UPGRADE_PLAN 批次 5。同日随手记：「禅模式」按用户要求更名「专注阅读」（i18n 单点，快捷键 F11 不变）；表格换行回归二次修复钉版（UP 2.4 补记：`table{width:auto;max-width:100%}` + `th/td{overflow-wrap:break-word}`，任何改动须真实文档截图验收） |
